...
export const SaveEmployee = ({ employees, type = SaveFormat.CSV }: SaveEmployeeProps) => {
    ...
    const convertToCSV = (data: EmployeeType[]) => {
        ...
        const rows = data.map(row => 
            Object.values(row)
                .map(value => 
                    typeof value === 'string' && value.includes(',') ? `"${value}"` : value // Handle commas in string values
                )
                .join(',')
        );
