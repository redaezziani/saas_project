...
const ProductsTable = () => {
    const { data: res, error } = useSWR("/api/products", fetcher, {
    ...
    const columns: ColumnDef<ProductType>[] = [
        ...
        },
        {
            ...
            header: "الاسم",
            cell: ({ row }) => (
                <div className="line-clamp-1 text-slate-600 dark:text-slate-100">
                    {row.getValue("name")}
                </div>
            ),
        },
        {
            ...
            header: "الوصف",
            cell: ({ row }) => (
                <div className="line-clamp-1 max-w-lg text-slate-600 dark:text-slate-100">
            ...
            cell: ({ row }) => (
                <div className="text-slate-600 dark:text-slate-100">
                    {row.getValue("currency") === "USD"
                        ? "دولار"
                        : row.getValue("currency") === "EUR"
                          ? "يورو"
                          : row.getValue("currency") === "MAD"
                            ? "درهم"
                            : row.getValue("currency") === "AED"
                              ? "درهم"
                              : row.getValue("currency") === "SAR"
                                ? "ريال"
                                : row.getValue("currency") === "QAR"
                                  ? "ريال"
                                  : row.getValue("currency") === "KWD"
                                    ? "دينار"
                                    : row.getValue("currency") === "BHD"
                                      ? "دينار"
                                      : row.getValue("currency") === "OMR"
                                        ? "ريال"
                                        : "غير معروف"}
                </div>
            ),
    ...
    if (!res)
        return (
            <div className="mt-10 flex h-96 w-full items-center justify-center gap-2 text-slate-400">
                <Loader2
                    size="20"
                    className="animate-spin text-slate-400 duration-300 ease-out"
                />
                <p>Loading...</p>
            </div>
        )
